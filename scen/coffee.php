...
if($hasNext){
    header("Location: ../coffeePage.php?q=" . $next_q);
    exit();
} else {
    $total_score = 0;
    foreach($_SESSION['answers'][$scenario_id] as $q_num => $answer_data){
        if(isset($answer_data['score'])){
    ...
    header("Location: ../coffeePage.php?result=1");
    exit();
}
